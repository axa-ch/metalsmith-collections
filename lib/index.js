...
function plugin(opts){
  ...
  return function(files, metalsmith, done){
    ...
    Object.keys(files).forEach(function(file){
      ...
      var matches = match(file, data)
      data.partOfCollections = matches;
...
function matcher(cols){
  ...
  var matchers = {};
  var attributeToCollection = {};

  keys.forEach(function(key){
    var opts = cols[key];
    if (opts.pattern)
      matchers[key] = new Matcher(opts.pattern);
    if (opts.hasAttribute)
      attributeToCollection[opts.hasAttribute] = key
  });
  ...
  return function(file, data){
    ...
    for (var attribute in attributeToCollection) {
      if (Object.prototype.hasOwnProperty.call(data, attribute)) {
          var collection = attributeToCollection[attribute];
